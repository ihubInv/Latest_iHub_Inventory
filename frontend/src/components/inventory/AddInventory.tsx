                              ...
                              <AssetNameDropdown
                                ...
                                onChange={async (value) => {
                                  ...
                                  if (value && it.assetcategory && formData.locationofitem) {
                                    // Get all existing unique IDs from other items in the batch (normalize to uppercase)
                                    const excludeIds = multipleItems
                                      .map((row, i) => i !== idx ? row.uniqueid : '')
                                      .filter(id => id && !id.includes('--'))
                                      .map(id => id.toUpperCase());
                                    const newUniqueId = await generateMultipleItemUniqueId(value, formData.locationofitem);
                        ...
                        onClick={() => {
                          ...
                          (async () => {
                            ...
                            if (!template.uniqueid) {
                              const newId = await generateMultipleItemUniqueId(asset, loc);
